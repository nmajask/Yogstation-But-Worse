...
/mob/living/carbon/human/proc/update_body_parts_head_only()
	...
	if(HD && !(HAS_TRAIT(src, TRAIT_HUSK)))
		...
		if(lip_style && (LIPS in dna.species.species_traits))
			var/mutable_appearance/lip_overlay = mutable_appearance(dna.species.face_icon, "lips_[lip_style]", -BODY_LAYER)
		...
		if(!(NOEYESPRITES in dna.species.species_traits))
			...
			if(!E)
				eye_overlay = mutable_appearance(dna.species.face_icon, "eyes_missing", -BODY_LAYER)
			else
				eye_overlay = mutable_appearance(dna.species.face_icon, E.eye_icon_state, -BODY_LAYER)
